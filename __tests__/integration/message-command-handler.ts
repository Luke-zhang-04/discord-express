...
describe("test message command handler", () => {
    ...
    client.messageCommand("myCommand", (request) => {
        expect(request.requestType).toBe("message")
        awaiter.call()
    })
    ...
    test("should call handler with message command", async () => {
    ...
    test("should not call handler with slash command", async () => {
